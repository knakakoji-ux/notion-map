...
H.append('<div class="lead">分母は小売総額 167兆円。下記は「受注形態（通販・EC）」「購買目的（ギフト）」「業態（DgS・百貨店）」というレイヤーの違う数字で、重複するため合計しない。</div>')
sizes=[
 ('通販（JADMA総額）',14.55,C_NEWS,'受注形態・EC含む／推計・2024年度・26年連続増'),
 ('物販EC（経産省）',15.20,C_EC,'受注形態／推計・2024・EC化率9.78%'),
 ('ギフト市場（矢野）',11.19,C_GIFT,'購買目的／推計・2024・ほぼ横ばい'),
 ('ドラッグストア（JACDS）',10.03,C_STORE,'業態／推計・2024年度・初の10兆円'),
 ('百貨店（協会）',5.77,'#8a6f9c','業態／確定・2024・ピーク比6割'),
]
H.append(hbars(sizes))
H.append('<div class="legend-note">各チャネルが小売総額167兆円に占める割合：通販8.7%／物販EC9.1%／ギフト6.7%／DgS6.0%／百貨店3.5%。'
         '<b>規模そのものは横並びでも、中身（誰が買うか・何が伸びるか）が決定的に違う</b>のが次節。</div>')
H.append('</div>')
...
H.append('<div class="lead">河野メリクロンの主力は「化粧品・育毛剤・健康食品」。各チャネルでこの領域がどれだけ売れているかが拡張余地の目安。</div>')
...
H.append('<div class="legend-note">'+tag('fact')+
         ...
         'DgS：JACDS2024年度（ビューティケア1兆8,272億円＝18.2%、調剤含む）。百貨店：協会2023（化粧品4,416億円＝8.1%）。'
         '<span class="note">EC内訳%は主要4カテゴリ実額を15.2兆で按分（残差をその他）。百貨店の衣料・食料は概況からの推計。</span></div>')
H.append('<p style="margin-top:12px"><b>含意：</b>化粧品・健食はECで約1.15兆円・DgSで約1.8兆円が動く成長カテゴリ。'
         '河野メリクロンが強い「育毛・スキンケア」は、ECモール（化粧品ECは楽天27.8%＋Amazon19.2%＝約47%が主戦場）とDgSビューティケアの両方に明確な売り場がある。</p>')
H.append('</div>')
...
series=[
 ...
 ('通販JADMA',[3.49,6.15,14.55],C_NEWS,'4 3'),
...
H.append('<div class="legend-note">'+tag('fact')+'＋'+tag('fermi')+
         '　EC物販：2014年 約6.8兆円→2024年 15.2兆円（経産省、2004年頃は黎明期で約3.5兆円・推計）。'
         '通販JADMA：2004年度 約3.5兆→2014年度 6.15兆→2024年度 14.55兆（推計・26年連続増、内訳はEC化で膨張）。'
         'ギフト：2015年 9.95兆→2024年 11.19兆（10年で+16%・ほぼ横ばい）。'
         'DgS：2008年度 5.23兆→2024年度 10.03兆。百貨店：1991年ピーク9.71兆→2024年 5.77兆（縮小、2004/2014年は推計補間）。</div>')
# eギフトの別枠
